...
module branch_rs #(parameter ROB = 32,RS = 4, W = 31, I = 7, C = 3)
    (input clk,
    input reset,
    ...
    input logic[C+3:0] op_control,
    /*Source operand values to choose from*/
    input logic[W:0] rs1, rs2, seq_pc,prediction_address,
    ...
    /*Prediction address*/
    output logic[W:0] branch_predicted_address,
    ...
    output logic[W:0] branch_seq_pc,
    ...
    logic[W:0] prediction_addr_storage[RS - 1:0];
    logic[C+3:0] decodeinfo_storage[RS - 1:0];
    ...
    always_comb begin
        ...
        for(int i = RS -1; i >= 0; i--)begin
            if(valid_storage[i] & ready1_storage[i] & ready2_storage[i] & !reset_pipeline)begin
                ...
                addr = prediction_addr_storage[i];
    ...
    always_ff@(posedge clk) begin
        if(reset)begin
            {branch_selected,branch_prediction_index,branch_ghr,branch_seq_pc,branch_op1} <= '0;
            {branch_op2,branch_rob,branch_mode,branch_predicted_address} <= '0;  
        end
        
        else begin
            branch_selected <= instr_found;
            branch_prediction_index <= p_index;
            branch_ghr <= ghr;
            branch_seq_pc <= nxt_pc;
            branch_op1 <= op1;
            branch_op2 <= op2;
            branch_rob <= rob;
            branch_mode <= mode;
            branch_predicted_address <= addr;
        end 
    end
