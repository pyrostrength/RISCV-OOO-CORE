...
module ri_rs_tb #(parameter W = 31,ROB = 32, C = 3, RS = 16)
    (
    ...
    /*Decoding info*/
    logic[C:0] decodeinfo_array[RS - 1:0];
    ...
    */
    ...
    task automatic check_rs(input logic[$clog2(ROB):0] removed_items[$]);
        ...
        assert ((ri_mode == mode) & (ri_selected == selected) & (ri_op1 == op1) 
            & (ri_op2 == op2) & (ri_rob == rob) & (rs_full == full))
        else begin
