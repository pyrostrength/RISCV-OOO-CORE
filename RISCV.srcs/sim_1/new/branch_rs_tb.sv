...
module branch_rs_tb #(parameter W = 31,ROB = 32, C = 3, RS = 4, I = 7)
    (
     ...
     logic[W:0] branch_predicted_address, branch_seq_pc;
     
     branch_rs branch_station(.*);
    ...
    /*Decoding info*/
    logic[C+3:0] decodeinfo_array[RS - 1:0];
    ...
    */
    task automatic run_rs(output logic[$clog2(ROB):0] removed_items[$]);
        ...
        rob isn't full*/
            ...
            if((first_open_entry.size()>0) & station_request & !full_rob)begin
                decodeinfo_array[first_open_entry[0]] = op_control;
            ...
            if(first_selected_entry.size() > 0)begin
                mode = decodeinfo_array[first_selected_entry[0]];
    ...
    task automatic check_rs(input logic[$clog2(ROB):0] removed_items[$]);
        ...
        assert ((branch_mode == mode) & (branch_selected == selected) & (branch_op1 == op1) 
            & (branch_op2 == op2) & (branch_rob == rob) & (rs_full == full) &
            (branch_prediction_index == pht_index ) & (branch_ghr == ghr_check ) 
            & (branch_predicted_address == addr ) & (branch_seq_pc == nxt_pc))
